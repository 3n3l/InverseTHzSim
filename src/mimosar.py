...
class Antenna:
    ...
    def gaussian_pattern_2d(self, incoming_dir):
        ...
        azimuth = torch.arccos(
            torch.Tensor(incoming_dir.z).to(config["device"])
            / torch.sqrt(
                torch.Tensor(incoming_dir.x).to(config["device"]) ** 2
                + torch.Tensor(incoming_dir.z).to(config["device"]) ** 2
            )
        )  # Azimuth angle (phi) in radians
        elevation = torch.arccos(
            torch.Tensor(incoming_dir.z).to(config["device"])
            / torch.sqrt(
                torch.Tensor(incoming_dir.y).to(config["device"]) ** 2
                + torch.Tensor(incoming_dir.z).to(config["device"]) ** 2
            )
...
class MIMOSAR:
    ...
    def switch_off(self, Tx_idx=None, Rx_idx=None):
        ...
        if Rx_idx:
            ...
            # NOTE: dr.count() will cast this to an UInt, item() returns 
            print("A:", type(self._N_Rx - dr.count(~self.Rx_mask)))  # pyright: ignore
            print("B:", type(self._N_Rx - dr.count(~self.Rx_mask)).item())  # pyright: ignore
            self.params["N_Rx"] = (self._N_Rx - dr.count(~self.Rx_mask)).item()  # pyright: ignore
