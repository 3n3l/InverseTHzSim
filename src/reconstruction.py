...
class RecGrid:
    ...
    def reconstruct(self, measurement):
        a = sparams.mimo_system.get_param("N_Tx")
        print(a, type(a))
        b = sparams.mimo_system.get_param("N_Rx")
        print(b, type(b))
        Tx = (
            torch.Tensor(dr.ravel(sparams.mimo_system.get_param("Tx")))
            .reshape([sparams.mimo_system.get_param("N_Tx"), 3])
            .to(config["device"])
        )
        Rx = (
            torch.Tensor(dr.ravel(sparams.mimo_system.get_param("Rx")))
            .reshape([sparams.mimo_system.get_param("N_Rx"), 3])
            .to(config["device"])
        )
