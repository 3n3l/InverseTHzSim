...
class Measurement:
    ...
    def add_noise(self, mag=0.1):
        ...
        ref_abs = torch.abs(msr) + mag * torch.rand(
            sparams.mimo_system.get_param("N_Tx"),
            sparams.mimo_system.get_param("N_Rx"),
            sparams.mimo_system.get_param("N_Fq"),
        ).to(config["device"])
...
class Real(Measurement):
    ...
    def print_statistics(self):
        ...
        for batch in range(n_batches):
            ...
            d = self.msr[i] - self.msr[j]) / torch.linalg.norm(torch.abs(self.msr[i]))
            diffs[i, j] = torch.sum(torch.abs(d, axis=(1, 2, 3)).reshape(-1, 1) # pyright: ignore
...
class Simulated(Measurement):
    def __init__(self, scene, ms=False):
        ...
        self.ctx_args = {"scene": scene.get_scene()}
