...
import manifold as sms
import visualization as viz


def connect_to_Rx(si, scene, Rx, indices):
    temp = Rx - dr.gather(type(si.p), si.p, mi.UInt(dr.arange(mi.UInt, 0, config["N_Paths"])))
    # temp = Rx - dr.gather(mi.Vector3f, mi.Vector3f(si.p), mi.UInt(dr.arange(mi.UInt, 0, config["N_Paths"])))
    ...
    if config["material"] == "rough":
        ...
        r = sms.reflect(dr.gather(mi.Vector3f, si.wi, indices), dr.gather(mi.Vector3f, norms, indices))
        if config["relax_physics"]:
            throughput = throughput * (
                (
                    config["phong_kd"]
                    * dr.dot(dr.gather(mi.Vector3f, si.wi, indices), dr.gather(type(si.n), si.n, indices))
                )
                + config["phong_ks"]
                * dr.select((dr.dot(r, wo) > 0.0), (dr.power(dr.dot(r, wo), config["phong_ns"])), 0.0)
            )
        else:
...
def get_bounce(si, tp, scene):
    ...
    if config["material"] == "rough":
        ...
        r = sms.reflect(dr.gather(mi.Vector3f, si.wi, indices), dr.gather(mi.Vector3f, si.n, indices))
        new_tp = new_tp * (
            (config["phong_kd"] * dr.dot(dr.gather(mi.Vector3f, si.wi, indices), dr.gather(mi.Vector3f, si.n, indices)))
            + config["phong_ks"]
            * dr.select((dr.dot(r, wo_local) > 0.0), (dr.power(dr.dot(r, wo_local), config["phong_ns"])), 0.0)
        )
...
def simulate_measurements_multi(ctx_args, depth=1):
    ...
    real = dr.zeros(mi.TensorXf, shape=[N_Tx, N_Rx, N_Fq])
    imag = dr.zeros(mi.TensorXf, shape=[N_Tx, N_Rx, N_Fq])
    ...
    for T_i in range(sparams.mimo_system.get_param("N_Tx")):
        ...
        origins = mi.Vector3f(
            np.repeat(Ti.x, config["N_Paths"]), np.repeat(Ti.y, config["N_Paths"]), np.repeat(Ti.z, config["N_Paths"])
        )
        ...
        tp = (
            dr.ones(mi.Float, config["N_Paths"])
            * (1 / pdf)
            * mi.Float(sparams.mimo_system.get_param("antenna").gaussian_pattern_2d(dr.normalize(-ray_d)))
        )
        if not config["relax_physics"]:
...
# Run Simulation
def simulate_measurements_ms(ctx_args, n_paths: int = 50):
    scene = ctx_args["scene"]
    ...
    for T_i in range(sparams.mimo_system.get_param("N_Tx")):
        ...
        for R_i in range(sparams.mimo_system.get_param("N_Rx")):
            ...
            # sms_success = False
            sms_success, v_final = sms.sample_manifold(Ti, Ri, scene, 0.02, 1e-5, 250)
            if not sms_success:
                print("SMS FAILED")
            ...
            # Fq_t = mi.TensorXf(dr.ravel(sparams.mimo_system.get_param("Fq")), shape=[sparams.mimo_system.get_param("N_Fq")])
            dist = mi.TensorXf(dr.ravel(result))
            amps = mi.TensorXf(dr.ones(mi.Float, n_paths))
            Fq_t = mi.TensorXf(dr.ravel(sparams.mimo_system.get_param("Fq")))
