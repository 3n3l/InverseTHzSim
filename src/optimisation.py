...
def mc_estimate(f_xi, p_xi):
    N = f_xi.shape[0]
    estimate = 1.0 / N * (f_xi / p_xi).sum(dim=0)  # average along batch axis, leave dimension axis unchanged
    return estimate
    ...
    def wrapper(input_tensor, context_args, *args):
        class SmoothedFunc(torch.autograd.Function):
            ...
            def forward(ctx, input_tensor, context_args, *args):
                ...
                forward_output = convolve(
                    grad_of_gaussiankernel,
                    func,
                    importance_fn,
                    input_tensor,
                    context_args["nsamples"],
                    context_args,
                    args,
                )
...
def run_optimization(hparams, optim, theta, gt_theta, ctx_args, schedule_fn, update_fn):
    ...
    print(f"Iter 0/{hparams['epochs']}, ParamLoss: {param_error}, " f"L2HammLoss: {msr_errors[-1]:.8f}")
    ...
    for j in range(hparams["epochs"]):
        ...
        with torch.no_grad():
            ...
            print(
                f"Iter {j + 1}/{hparams['epochs']}, ParamLoss: {param_error}, "
