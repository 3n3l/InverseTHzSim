...
def loss_landscape_2D(
    ref, x_axis, y_axis, scene, transform, losses, titles, x_label="X", y_label="Y", compute=True, save=False
):
    path = setup_utils.save_path()
    cal_path = setup_utils.cal_path()
    hist = [[] for _ in range(len(losses))]
...
def find_min_2D(title, x_axis, y_axis):
    loaded = np.fromfile(f"../data/loss_landscapes/2D/losses2D_{title}.dat", dtype=float).reshape(
...
def compute_loss_landscape(
    ...
    if config["op_dim"] == 1:
        ...
        loss_landscape_1D(
            ref_cpx, x_axis, scene, update_fn, loss_fns, titles, x_label=x_label, compute=compute, save=True
        )
