...
def convert_rdm_to_data(rdm):
    real = np.zeros((sparams.mimo_system.get_param('N_Tx'), sparams.mimo_system.get_param('N_Rx'), sparams.mimo_system.get_param('N_Fq')))
    imag = np.zeros((sparams.mimo_system.get_param('N_Tx'), sparams.mimo_system.get_param('N_Rx'), sparams.mimo_system.get_param('N_Fq')))
    for Tx in range(sparams.mimo_system.get_param('N_Tx')):
        for Rx in range(sparams.mimo_system.get_param('N_Rx')):
            for fq in range(sparams.mimo_system.get_param('N_Fq')):
        ...
        for Tx in range(sparams.N_Tx):
            for Rx in range(sparams.N_Rx):
                for fq in range(sparams.N_Fq):
                    idx = Y * sparams.N_Rx * sparams.N_Tx * sparams.N_Fq + Rx * sparams.N_Tx * sparams.N_Fq + Tx * sparams.N_Fq + fq
